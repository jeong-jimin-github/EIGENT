...
// ============================================================
// Git operations — IPC in Electron, Hono API in browser mode
// ============================================================

export async function fetchGitBranches(directory: string): Promise<GitBranchInfo> {
	if (isElectron) return window.palot.git.listBranches(directory)
	const { fetchGitBranches: httpFetch } = await import("./palot-server")
	return httpFetch(directory) as Promise<GitBranchInfo>
}

export async function fetchGitStatus(directory: string): Promise<GitStatusInfo> {
	if (isElectron) return window.palot.git.getStatus(directory)
	const { fetchGitStatus: httpFetch } = await import("./palot-server")
	return httpFetch(directory) as Promise<GitStatusInfo>
}

export async function gitCheckout(directory: string, branch: string): Promise<GitCheckoutResult> {
	if (isElectron) return window.palot.git.checkout(directory, branch)
	const { gitCheckout: httpCheckout } = await import("./palot-server")
	return httpCheckout(directory, branch) as Promise<GitCheckoutResult>
}
...
export async function gitStashAndCheckout(
	...
	if (isElectron) return window.palot.git.stashAndCheckout(directory, branch)
	const { gitStashAndCheckout: httpStashAndCheckout } = await import("./palot-server")
	return httpStashAndCheckout(directory, branch) as Promise<GitStashResult>
}

export async function gitStashPop(directory: string): Promise<GitStashResult> {
	if (isElectron) return window.palot.git.stashPop(directory)
	const { gitStashPop: httpStashPop } = await import("./palot-server")
	return httpStashPop(directory) as Promise<GitStashResult>
}
...
export async function getGitRoot(directory: string): Promise<string | null> {
	if (isElectron) return window.palot.git.getRoot(directory)
	const { getGitRoot: httpGetRoot } = await import("./palot-server")
	return httpGetRoot(directory)
}

export async function fetchDiffStat(directory: string): Promise<GitDiffStat> {
	if (isElectron) return window.palot.git.diffStat(directory)
	const { fetchDiffStat: httpFetch } = await import("./palot-server")
	return httpFetch(directory) as Promise<GitDiffStat>
}

export async function gitCommitAll(directory: string, message: string): Promise<GitCommitResult> {
	if (isElectron) return window.palot.git.commitAll(directory, message)
	const { gitCommitAll: httpCommitAll } = await import("./palot-server")
	return httpCommitAll(directory, message) as Promise<GitCommitResult>
}

export async function gitPush(directory: string, remote?: string): Promise<GitPushResult> {
	if (isElectron) return window.palot.git.push(directory, remote)
	const { gitPush: httpPush } = await import("./palot-server")
	return httpPush(directory, remote) as Promise<GitPushResult>
}
...
export async function gitCreateBranch(
	...
	if (isElectron) return window.palot.git.createBranch(directory, branchName)
	const { gitCreateBranch: httpCreateBranch } = await import("./palot-server")
	return httpCreateBranch(directory, branchName) as Promise<GitCheckoutResult>
}

export async function getGitRemoteUrl(directory: string, remote?: string): Promise<string | null> {
	if (isElectron) return window.palot.git.getRemoteUrl(directory, remote)
	const { getGitRemoteUrl: httpGetRemoteUrl } = await import("./palot-server")
	return httpGetRemoteUrl(directory, remote)
}
...
export async function gitApplyToLocal(
	...
	if (isElectron) return window.palot.git.applyToLocal(worktreeDir, localDir)
	const { gitApplyToLocal: httpApplyToLocal } = await import("./palot-server")
	return httpApplyToLocal(worktreeDir, localDir) as Promise<GitApplyResult>
}
...
export async function gitApplyDiffText(
	...
	if (isElectron) return window.palot.git.applyDiffText(localDir, diffText)
	const { gitApplyDiffText: httpApplyDiffText } = await import("./palot-server")
	return httpApplyDiffText(localDir, diffText) as Promise<GitApplyResult>
}
