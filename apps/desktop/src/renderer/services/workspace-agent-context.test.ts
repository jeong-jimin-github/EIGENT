...
describe("workspace agent context", () => {
	...
	test("pins No Project work and documents both preview paths", () => {
		...
		expect(prompt).toContain("/api/workspace/preview-reload")
		expect(prompt).toContain("curl -fsS -X POST")
		expect(prompt).toContain('"root":"/var/lib/eigent/workspaces/_no-project"')
		expect(prompt).toContain("Do not ask the user to press refresh")
